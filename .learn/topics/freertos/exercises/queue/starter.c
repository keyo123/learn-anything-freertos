...
static void vTaskReceiver(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        // BaseType_t xReturn = xQueueReceive( ... );
...
static void vTaskQueueFullTester(void *pvParameters)
{
    ...
    printfSafe("[Tester] Starting to send 4 messages to the queue (Queue capacity is 3)...\r\n");
    for (i = 1; i <= 4; i++)
    {
    ...
    /* 恢复接收任务，清空队列 */
    printfSafe("[Tester] Resuming Receiver to process accumulated messages...\r\n\r\n");
...
void start_queue_demo(void)
{
    ...
    if (xTestQueue != NULL)
    {
        ...
        /* 创建发送者任务：优先级 1 */
        xTaskCreate(vTaskSender, "Sender", configMINIMAL_STACK_SIZE * 2, NULL, 1, &xSenderHandle);

        /* 创建接收者任务：优先级 2 (比发送者高，会立即抢占接收) */
        xTaskCreate(vTaskReceiver, "Receiver", configMINIMAL_STACK_SIZE * 2, NULL, 2, &xReceiverHandle);

        /* 创建测试者任务：优先级 3 */
        xTaskCreate(vTaskQueueFullTester, "Tester", configMINIMAL_STACK_SIZE * 2, NULL, 3, &xTesterHandle);
    }
    else
    {
