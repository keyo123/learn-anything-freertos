...
static void vTaskSensorAlert(void *pvParameters) {
  ...
  printfSafe(
      "[SensorAlert] High Temperature detected! Sending alert bit...\r\n");
  /* TODO: 使用 xTaskNotify 向处理器任务发送通知，动作选择 eSetBits，值选择
   * BIT_TEMP_ALERT */
  xTaskNotify(xAlertProcessorHandle, BIT_TEMP_ALERT, eSetBits);
  ...
  /* TODO: 使用 xTaskNotify 向处理器任务发送通知，动作选择 eSetBits，值选择
   * BIT_HUMID_ALERT */
  xTaskNotify(xAlertProcessorHandle, BIT_HUMID_ALERT, eSetBits);

  vTaskDelay(pdMS_TO_TICKS(2000));

  /* 3. 同时触发温湿度警报 (置位 BIT_TEMP_ALERT | BIT_HUMID_ALERT) */
  printfSafe("[SensorAlert] CRITICAL! Temp and Humid abnormal! Sending alert "
             "bits...\r\n");
  /* TODO: 发送包含温湿度两个标志位的通知 */
  xTaskNotify(xAlertProcessorHandle, BIT_TEMP_ALERT | BIT_HUMID_ALERT, eSetBits);
...
/**
 * @brief  数据源任务：模拟实时传输 ADC 数据值，并使用 eSetValueWithOverwrite
 * 覆写通知值
 */
static void vTaskSensorData(void *pvParameters) {
  (void)pvParameters;
  ...
  for (;;) {
    ...
    printfSafe("[SensorData] Realtime ADC value sampled: %d. Sending to "
               "processor...\r\n",
               (int)adcRawValue);

    /* TODO: 使用 xTaskNotify 向处理器任务发送数值，动作选择
     * eSetValueWithOverwrite */
    xTaskNotify(xAlertProcessorHandle, adcRawValue, eSetValueWithOverwrite);

    /* 每隔 3000 毫秒（3秒）发送一次实时 data */
...
static void vTaskAlertProcessor(void *pvParameters) {
  ...
  for (;;) {
     ...
     * - ulBitsToClearOnExit: 退出时将通知值的所有 32 位全部清零 (设为
     * 0xFFFFFFFF)
    ...
    if (xReturn == pdPASS) {
      /* TODO: 判断通知值中是否包含警报位 */
      /* 提示: 如果 ulNotifiedValue
       * 的警报位非零，说明这是一条警报事件；否则是一条 ADC 实时数据值 */
      if (ulNotifiedValue & (BIT_TEMP_ALERT | BIT_HUMID_ALERT)) {
        printfSafe(
            ">>> [Processor] !!! ALERT RECEIVED !!! StatusBits: 0x%02X\r\n",
...
void start_notify_adv_demo(void) {
  ...
  xTaskCreate(vTaskAlertProcessor, "Processor", configMINIMAL_STACK_SIZE * 2,
              NULL, 2, &xAlertProcessorHandle);

  if (xAlertProcessorHandle != NULL) {
    /* 2. 创建发送警报任务，优先级为 1 */
    xTaskCreate(vTaskSensorAlert, "SensorAlert", configMINIMAL_STACK_SIZE * 2,
                NULL, 1, &xSensorAlertHandle);

    /* 3. 创建发送实时数据任务，优先级为 1 */
    xTaskCreate(vTaskSensorData, "SensorData", configMINIMAL_STACK_SIZE * 2,
                NULL, 1, &xSensorDataHandle);
