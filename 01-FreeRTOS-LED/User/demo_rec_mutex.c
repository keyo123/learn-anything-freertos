...
static void print_nested_log(int depth) {
  ...
  if (xSemaphoreTakeRecursive(xRecMutex, portMAX_DELAY) == pdTRUE) {
    ...
    /* TODO: 释放递归互斥量 */
    if (depth > 1) { // 👈 只有深度大于1时才释放，最内层（depth == 1）少释放一次！
...
void start_rec_mutex_demo(void) {
  ...
  if (xRecMutex != NULL) {
    xTaskCreate(vTaskRecursiveDemo, "RecDemo", configMINIMAL_STACK_SIZE * 2,
                NULL, 2, &xRecDemoTaskHandle);

    /* 实验 B 开启：创建观察者任务测试成对释放 */
    xTaskCreate(vTaskObserver, "Observer", configMINIMAL_STACK_SIZE, NULL, 1,
                &xObserverTaskHandle);
  }
}
