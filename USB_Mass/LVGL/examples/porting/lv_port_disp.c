...
/* LCD SPI 互斥量 — 保护 ST7789 刷屏不被多任务并发中断 */
...
void lv_port_disp_init(void)
{
    ...
    static lv_color_t buf_1[MY_DISP_HOR_RES * 4];                          /*A buffer for 4 rows*/
    ...
    if(disp_flush_enabled)
    {
        ...
        for(y = area->y1; y <= area->y2; y++) {
            for(x = area->x1; x <= area->x2; x++) {
                ...
                /*put_px(x, y, *color_p)*/
	//				colorbit = (uint16_t)color_p;
					LCD_DrawPoint(x,y,color_p->full);
					color_p++;
	//				LCD_Fill(x,y,x,y,color_p->full);
            }
        }
#else
		uint32_t y = 0;
...
//    int32_t x, y;
//    for(y = fill_area->y1; y < fill_area->y2 + (fill_area->x2 - fill_area->x1); y++) {
//        for(x = fill_area->x1; x < fill_area->x2; x++) {
//            dest_buf[y * dest_width + x] = color;
//        }
//    }
//}
